...
// + Logging to console and file, graceful shutdown (SIGINT)
// Build: make
...
static const char* dir_str(dir_t d){
    switch(d){ case DIR_N: return "N"; case DIR_E: return "E"; case DIR_S: return "S"; case DIR_W: return "W"; }
    return "N";
}
...
static void broadcast_tlm(void){
    ...
    pthread_mutex_unlock(&g_state_mx);
    int n=snprintf(line,sizeof(line),"TLM speed=%d;battery=%d;temp=%d;dir=%s;ts=%ld\n",
                   sp,bt,tp,ds,(long)now);

    pthread_mutex_lock(&g_clients_mx);
    for(client_t *c=g_clients;c;c=c->next) if(c->fd>=0) send(c->fd,line,(size_t)n,0);
    pthread_mutex_unlock(&g_clients_mx);
}
            ...
            *nl='\0'; size_t L=strlen(p); if(L&&p[L-1]=='\r') p[L-1]='\0';
            ...
            if(strcmp(p,"QUIT")==0){
                ...
                if(s.role!=ROLE_ADMIN) dprintf(s.fd,"ERR forbidden\n");
                       ...
                       pthread_mutex_lock(&g_state_mx); const char* ds=dir_str(g_dir); pthread_mutex_unlock(&g_state_mx);
...
int main(int argc, char **argv){
    ...
    g_logf = fopen(argv[2], "a"); /* if NULL, we'll log only to stderr */
