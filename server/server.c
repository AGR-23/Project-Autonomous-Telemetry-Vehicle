// minimal TCP listener + one thread per client
// Build: make
...
static void *client_thread(void *arg) {
    ...
    send(c->fd, "OK Welcome. Commands later...\n", 30, 0);
    close(c->fd);
    free(c);
    return NULL;
}
...
int main(int argc, char **argv) {
    ...
    for (;;) {
        ...
        client_t *ctx = calloc(1,sizeof(*ctx)); ctx->fd=cfd; ctx->addr=cli;
        pthread_t th; pthread_create(&th,NULL,client_thread,ctx); pthread_detach(th);
    }
    close(sfd);
    return 0;
}